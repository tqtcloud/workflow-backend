...
make脚手架
```sh
➜  workflow-backend git:(master) ✗ make help
...
2. 添加配置文件(默认读取位置: etc/workflow-backend.toml)
```sh
$ 编辑样例配置文件 etc/workflow-backend.toml.book
$ mv etc/workflow-backend.toml.book etc/workflow-backend.toml
```
